-- Infraestrutura de mensagens internas (chat + anexos)
...
create index if not exists idx_mensagens_demanda on public.mensagens(demanda_id);
create index if not exists idx_anexos_mensagens_msg on public.anexos_mensagens(mensagem_id);
...
drop policy if exists mensagens_update_public on public.mensagens;
create policy mensagens_update_public
on public.mensagens
for update
to public
using (true)
...
drop policy if exists mensagens_delete_public on public.mensagens;
create policy mensagens_delete_public
on public.mensagens
for delete
to public
using (true);
...
drop policy if exists anexos_mensagens_update_public on public.anexos_mensagens;
create policy anexos_mensagens_update_public
on public.anexos_mensagens
for update
to public
using (true)
...
drop policy if exists anexos_mensagens_delete_public on public.anexos_mensagens;
create policy anexos_mensagens_delete_public
on public.anexos_mensagens
for delete
to public
using (true);

-- Bucket de anexos (idempotente)
